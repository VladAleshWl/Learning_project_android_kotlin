...
class koloda : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        fun vipoln_sapolnenia() {
            i_for_pole = 0
            for (karta__a in basa_kart.dinamic_nabor_kart) { knopki_and_vivod(karta__a) }
            for (karta__a in basa_kart.dinamic_nabor_kart_vragi) { knopki_and_vivod(karta__a) }
            for (karta__a in basa_kart.dinamic_nabor_kart_bos) { knopki_and_vivod(karta__a) }             //перебор всех карт в игре
        ...
        kn_bli.image.setOnClickListener{
            ...
            kn_ipi.image.isClickable = kn_bli.flag_1

        }
        kn_dal.image.setOnClickListener{
            knopki_click(kn_dal)
            kn_bli.image.isClickable = kn_dal.flag_1
            kn_ipi.image.isClickable = kn_dal.flag_1
        }
        kn_ipi.image.setOnClickListener{
            knopki_click(kn_ipi)
            kn_dal.image.isClickable = kn_ipi.flag_1
            kn_bli.image.isClickable = kn_ipi.flag_1
        }
